  ...
  fetch(`/imdb/${movieID}`)
    ...
    .then(formattedData => {
      fetch('/profile', {
        ...
        .then(res => res.json())
        .then(data => console.log('Poster click, Post movie to db:', data))
        .catch(err => {
          console.log('Error in poster click, posting movie to db: ', err)
        })
    })
}
  ...
  if (index > movie.length-1) return <div className='poster'></div>
    ...
    return (
      <div className='poster'>
        ...
        <button className='add poster' onClick={() => handleClick(movieID)}>
